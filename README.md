...
The session itself is the **orchestrator** (`core.md`): it thinks, decomposes work, delegates the work to the role agents, verifies and consolidates what they return, and is the only one that talks to the user or publishes. The test is attention, not size — anything that would absorb it belongs to an agent; a typo it just fixes. Each agent runs in its own isolated context, with a `tools:` allowlist as its outer limit and its definition as the role it keeps inside that limit:
...
| 🔍 [`reviewer`](agents/reviewer.md) | `pr-review-toolkit` review agents · `/security-review` on demand (independent scrutiny) | **no Write/Edit**, no Linear | reports ranked findings; never patches |
| 🔬 [`researcher`](agents/researcher.md) | `WebSearch` · `WebFetch` (web/external questions, cited answers) | **no Write/Edit**, no Linear | answers with citations; no repo changes |
| 🧭 [`lead`](agents/lead.md) | `orchestrating-subagents` (owns a big multi-role goal end-to-end, runs the workers itself) | **no Write/Edit**, no Linear | pure orchestrator; delegates + verifies, authors nothing |
...
**The roles orchestrate too, one level down.** Work that splits into independent parts defaults to one agent per part, and every role but the PM carries `Agent` so it can fan out *within its own role* — sub-developers on independent parts of a fix, one reviewer per lens, parallel research angles — converging the results itself. The cap is **two levels**: the session spawns agents, those agents may spawn agents, and there it stops (spawning a `pr-review-toolkit` review agent as a gate is exempt — reviewing is a gate, not a delegation). For a big, clear, multi-role goal, the session can instead hand the *whole* goal to a **`lead`** — a semi-orchestrator that owns it end-to-end and runs the worker roles itself (level-1; its workers are the leaves) — cutting the session's back-and-forth and freeing it to drive several big streams at once. The PM is deliberately without `Agent` — parallel board writes are how a board drifts.

The built-ins (`Explore`, `Plan`, `general-purpose`) cover generic search/planning — the custom roles exist for the skills and boundaries above, and every agent cleans up its own processes ([`hooks/spawn-managed.sh`](hooks/spawn-managed.sh) + [`hooks/reap-managed.sh`](hooks/reap-managed.sh) guarantee it). The full playbook — when to delegate, briefs, lane tracking and the teammate mailbox, worktree isolation — is the `orchestrating-subagents` skill.
...
- points `statusLine` and all hooks in `~/.claude/settings.json` at [`hooks/`](hooks/) — a jq merge that replaces toolkit-managed entries, preserves everything else, and backs the original up to `~/.claude/backups/` first — and sets two env vars: `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` (named teammates and `SendMessage`, the coordination layer the orchestrator uses) and `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH=3` (as of Claude Code 2.1.217 a subagent can't spawn one at all by default — this restores the two-level delegation model above *and* the one-deeper review gate a level-2 developer must still run, and nothing beyond it);
...
- runs a doctor: every wired path must exist and be executable, agents fully copied, and the `pr-review-toolkit` review plugin flagged if it's enabled but not yet installed (the manual step below).
...
- [`hooks/guard-config.sh`](hooks/guard-config.sh) (PreToolUse · Write/Edit) — approval prompt for device config (`~/.claude/*`, `~/.gitconfig`, `/etc/*`) and for the guard hooks themselves. Claude's own memory (`~/.claude/projects/**`), backups, and paths that resolve into a repo workspace stay open — though a write into the toolkit checkout from a session whose cwd is *outside* it now prompts (edits from inside stay free), so the instruction surface is seen, not slipped in from an unrelated repo; paths are realpath-resolved so a symlink can't dodge the gate in either direction.
...
It also lists three **working directories** (`permissions.additionalDirectories`), because anything outside the workspace prompts even in auto mode — which stalls a background agent on a human who isn't watching. A path inside a working directory is approved *before* any allow-rule is consulted, and for writes as much as reads: `~/.claude` (plugin, agent and skill definitions, project memory), the per-uid scratchpad root `/tmp/claude-<uid>` every session is told to use for temporary files, and this checkout (the skills are symlinks into it, and access is judged on the resolved path — the symlink directory alone isn't enough). Honest about the widening: this replaces five narrow read-only rules, and a directory covers everything under it, the session transcripts included — with four carve-outs, `permissions.deny` rules on `~/.claude/.credentials.json`, on `~/.claude/settings*.json` (both `settings.json` and the machine-local `settings.local.json`, which on another machine carry MCP tokens or API keys in their `env` block — travelling across machines is the whole premise), on the historical copies of it under `~/.claude/backups/settings.json.*`, and on `~/.claude/backups/.claude.json.backup.*` (Claude Code's own config backups, which carry the same `mcpServers` secrets). They stick because deny is evaluated *before* the working-directory check, and they govern the **Read tool** alone — the doctor and the statusline reach `settings.json` through `jq` and Python, and a shell read stays open, so nothing in the toolkit loses its view of it. What it does cost is the Edit tool, which refuses a file it hasn't read: `settings.json` changes through `install.sh` (where it belongs) or through Bash, and the builtin `/statusline` command will refuse. No *guard* is loosened, though — `PreToolUse` hooks run ahead of permission rules either way, so `guard-config.sh` above still prompts on every write into `~/.claude`, `guard-git.sh` on publishing and destructive commands, and `guard-linear.sh` on Linear writes.
...
The version segment (`⬡ v<count>·<sha>`) is your "are my changes applied?" light. `install.sh` stamps it at each full install from the toolkit's own git (`git rev-list --count` + short SHA — auto, no manual bumping); the statusline reads that stamp and appends a `⚠` when the repo's HEAD has since moved past it — i.e. you committed and haven't re-run `./install.sh` — or a dim `?` when it can't verify (git timed out or the sha wouldn't resolve), since the light must fail safe rather than show a confident clean stamp it hasn't earned (mirroring `segment_git`'s `?`). A verified match shows no marker. (Skills and `core.md` are live through the symlink regardless; the `⚠` nudges re-applying the *copied* pieces — agents and `settings.json`.)

...
- points `statusLine` and all hooks in `~/.claude/settings.json` at [`hooks/`](hooks/) — a jq merge that replaces toolkit-managed entries, preserves everything else, and backs the original up to `~/.claude/backups/` first — and sets `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` (named teammates and `SendMessage`, the coordination layer the orchestrator uses) plus `CLAUDE_CODE_ENABLE_TASKS=0`, which restores `TodoWrite` — the `Ctrl+T` checklist, off by default since Claude Code 2.1.142 — so the lead keeps a visible, updating list of its lanes, trading the shared team board whose `TaskCreate` reaches only teammates and so renders empty for the orchestrator anyway;

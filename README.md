...
```bash
for s in software-development feature-spec feature-milestones feature-develop doc-style backlog plannotator-loop project-bootstrap ui-review-loop; do
...
- Division of labor: [`CLAUDE.md`](CLAUDE.md) (here, imported by a one-line device pointer at `~/.claude/CLAUDE.md`) holds always-on hard constraints (git rules) plus the system-of-record rule — session learnings are codified into the toolkit, never duplicated into device config or repos; **these skills** hold procedures; **per-project docs** (written by `project-bootstrap`) hold each repo's visible contract.

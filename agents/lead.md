...
- **Two-level cap — your workers are the last level.** You're a level-1 orchestrator the main agent spawned; the workers you spawn are the leaves and **do not spawn further** — say so in every brief (a worker spawning `pr-review-toolkit` review agents as its review gate doesn't count). You're spawned by the main agent, never nested under another `lead`.

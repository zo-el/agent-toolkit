...
The reverse of mirroring out: read what actually happened and true the board up, using the write mechanics in this same skill (the preview-diff Discipline, the as-built comment, the state map, cycles, the gotchas). Run it at the PM cap's session start, whenever asked what's out of sync / to check or update statuses / to reconcile after work merged, and before `next-up`.

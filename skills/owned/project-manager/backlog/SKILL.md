...
- **Active work never feeds the backlog.** Anything noticed about a milestone or feature currently in flight is **decided inside that milestone**: folded into an existing task, added to the round as a task (often its testing phase), or explicitly dropped. During the round it may park as a note in a task's **Anything else** section — but at round close those notes get the same decision (next round, or dropped), never an automatic slide into the queue. The backlog holds only what's **unrelated to any active effort** — otherwise every round quietly grows it.

...
**Lead every lane name with its number.** Lanes are numbered sequentially, so open each `TaskCreate` subject with `#N ` — the next number — e.g. `#14 Rebase onto develop`. The number then rides with the lane everywhere it's referenced (chat, completion notifications, the Linear mirror), not only the task tool's own `#N.` rendering, so any lane is unambiguous to point at.
...
**Check, don't assume — the task tools may or may not be present in a given session.** Whichever of them your role carries, they are gated as a set on a server-side flag keyed to the model, so they can appear or vanish between sessions — even mid-session — with no config change on your side. Look before you lean on them. **When they're available, using them is the default, not a choice — always open a lane per stream** (by the rules above), so every stream the user should be able to see is a real lane, never left only in prose. The platform **persists that lane list on disk per session** (`~/.claude/tasks/<session-id>/`, one JSON file per task), so what survives for the platform to restore on a clean resume is the *task*, not the teammate that was working it (ephemeral by design — the mailbox caveats below); rely on that platform persistence and build no restore layer of your own. **When they're genuinely absent:** surface the same lanes in your message to the user — a short, scannable list of the streams in flight and where each stands, refreshed as they move; same ownership, same altitude, only the medium changes. Either way: never try to fix their absence locally — no local setting reaches that gate — and never claim a list you have no tool to write.
...
- **Name the tier in the brief, and keep the two knobs agreed.** When a change is mechanical and low-risk, say so when you spawn — so the developer keeps its verification light (`develop`'s blast-radius gate: a self-read + a focused test, no reviewer fan, no local CodeRabbit) instead of running the full loop on glue. The agent's *review weight* and your *model* choice should name the same tier: a wrapper tagged mechanical drops to `model: sonnet` **and** reviews light; a risky change stays on the strong model **and** runs the full fan. (Frontmatter `effort:` is fixed per agent, not per-spawn; when effort itself must vary, run the work as a *workflow*, where `agent()` takes a per-stage `effort`.)
...
- **Reap before you return — a spawner isn't done while its spawns run.** Spawning is async: a one-shot fires a single completion notification when it finishes, a named teammate idles until you `TaskStop` it. Either way, **don't treat your own work as done — don't return — until every sub-agent you spawned has reported or been `TaskStop`ped.** This binds **any** agent that spawns, not only the main orchestrator: a developer that fanned out `pr-review-toolkit` reviewers waits for them before it hands back ready-for-review, exactly as you wait for a teammate here. (Why it matters, and the principle: `core.md` § Default posture.)

...
| 3 — Develop     | `feature-develop`              | the milestone's code + tests, review-clean        | every AC met, all suites green                                     |
| 4 — Finalize    | `feature-develop` § Phase 4    | PRs, changelogs, spec sync, milestone deleted     | merged                                                             |
...
- **Cross-cutting, always:** `doc-style` for any markdown; `plannotator-loop` for any substantial artifact review.
- **Artifacts are self-contained — never cross-coupled.** Every spec doc, milestone, and backlog item states what it needs in its own words and tracks only its own work. No file points at another's status, open work, or rationale — in either direction (not spec→milestone, milestone→backlog, backlog→backlog). The test: *finishing or changing one file must never force an edit to another* — if it does, the other file was tracking work it doesn't own, so fix the coupling, not the reference. A **completed milestone is frozen** — never edited again, so it holds no pointer to live or other work.
